...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate }) => {
  ...
  const menuItems = [
    ...
    { id: 'SETTINGS', label: 'Ajustes', show: true, icon: (
  ...
  return (
    <aside 
      ...
      // Quitamos el justify-between y dejamos que flex-col maneje el espacio
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] flex flex-col space-y-3 px-3">
        {menuItems.map((item) => 
          item.show && (
            <button 
              key={item.id}

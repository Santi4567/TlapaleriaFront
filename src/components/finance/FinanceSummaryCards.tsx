...
import { DollarSign, TrendingDown, TrendingUp, Receipt, PackageOpen, Award } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-8">
      ...
      <div className="bg-[#1a1a1a]/80 backdrop-blur-md border border-gray-800 rounded-2xl p-6 shadow-lg flex flex-col justify-between transition-all hover:border-gray-700">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-gray-400 font-semibold text-sm uppercase tracking-wider">Ventas Netas</h3>
        </div>
        <div>
          <p className="text-3xl font-extrabold text-white">{formatCurrency(data.netSalesAmount)}</p>
          <p className="text-xs text-gray-500 mt-2">Ingreso real a caja</p>
        </div>
      </div>

      {/* Tarjeta: Costo de Ventas (Inversión / Proveedor) */}
      <div className="bg-[#1a1a1a]/80 backdrop-blur-md border border-gray-800 rounded-2xl p-6 shadow-lg flex flex-col justify-between transition-all hover:border-gray-700">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-amber-500/10 flex items-center justify-center border border-amber-500/20">
            <PackageOpen className="w-5 h-5 text-amber-400" />
          </div>
          <h3 className="text-gray-400 font-semibold text-sm uppercase tracking-wider">Costo de Proveedor</h3>
        </div>
        <div>
          <p className="text-3xl font-extrabold text-white">{formatCurrency(data.costOfGoodsSold)}</p>
          <p className="text-xs text-gray-500 mt-2">Costo total de mercancía vendida</p>
        </div>
      </div>

      {/* Tarjeta: Ganancia Real */}
      <div className="bg-gradient-to-br from-[#1a1a1a] to-[#161616] border border-orange-500/30 rounded-2xl p-6 shadow-[0_4px_20px_rgba(249,115,22,0.1)] flex flex-col justify-between relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 mb-4 relative z-10">
          ...
          <h3 className="text-orange-400 font-bold text-sm uppercase tracking-wider">Ganancia Real</h3>
        </div>
        <div className="relative z-10">
          <p className="text-4xl font-black text-white">{formatCurrency(data.realProfitAmount)}</p>
          <p className="text-xs text-orange-500/70 mt-2 font-medium">Tu utilidad neta libre</p>
        </div>
      </div>
